...
def calculate():
    try:
        ...
        if not isinstance(selected_list, list):
            raise ValueError("Invalid data format for selected_list")
    ...
    if request.method == 'GET':
        try:
            ...
            selected_list = [
                {
                    'food_code': ing.food_code,
                    'grams': next((item['grams'] for item in session['selected_list'] if item['food_code'] == ing.food_code), 100),
                    'name': ing.name
                }
                for ing in ingredients
            ]
            ...
            # 全食材リストを取得（セレクトボックス用）
            all_ingredients = [
                {'food_code': ing.food_code, 'name': ing.name}
...
def get_ingredients():
    try:
        ingredients = Ingredient.query.all()
        return jsonify({"ingredients": [ing.to_dict() for ing in ingredients]})
    except Exception as e:
        print(f"Error in GET /ingredients: {e}")
        return jsonify({"error": str(e)}), 500
